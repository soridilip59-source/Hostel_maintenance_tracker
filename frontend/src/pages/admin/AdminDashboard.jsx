...
function AdminDashboard() {
  ...
  const counts = { total: requests.length, pending: requests.filter((item) => item.status === "Pending").length, progress: requests.filter((item) => item.status === "In Progress" || item.status === "In Process").length, resolved: requests.filter((item) => item.status === "Resolved").length, rejected: requests.filter((item) => item.status === "Rejected").length };
  ...
  return <>
    ...
    {loading ? <div className="loading-state">Loading dashboard data...</div> : <>
      <div className="stats-grid stats-grid-five"><StatCard icon="◈" label="Total Complaints" value={counts.total} trend="Live data" /><StatCard icon="◷" label="Pending" value={counts.pending} trend="Needs attention" /><StatCard icon="◒" label="In Process" value={counts.progress} trend="Being handled" /><StatCard icon="✓" label="Resolved" value={counts.resolved} trend="Completed" /><StatCard icon="×" label="Rejected" value={counts.rejected} trend="Closed" /></div>
